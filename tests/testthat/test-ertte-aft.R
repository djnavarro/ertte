test_that("ertte_aft() reproduces direct survreg() results", {
  ...
  expect_s3_class(mod, "ertte_aft")
  expect_s3_class(mod, "ertte_model")
  expect_s3_class(mod, "survreg")
  expect_identical(mod$ertte$type, "weibull")
})

test_that("ertte_aft() validates dist", {
  expect_error(
    ertte_aft(survival::Surv(time, event) ~ aucss, ertte_data, dist = "not_a_dist"),
    "`dist`"
  )
})
