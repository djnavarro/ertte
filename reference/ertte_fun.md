...
Takes a fitted ertte model as input and returns a function that
evaluates the survival function `S(t)` at user-specified parameters,
data, and times (e.g. for VPCs or other counterfactual simulation
scenarios). Named `ertte_fun()` for consistency with the companion
`erglm`/`emaxnls` packages' `erglm_fun()`/`emax_fun()`, which serve the
same purpose for their respective model classes. The returned function
checks that `param` is numeric and has one entry per column of the model
...
from `param` (the coefficient vector from
[`coef()`](https://rdrr.io/r/stats/coef.html) never includes it).
