...
#'
#' @param formula Model formula, as for [ertte_model()]
#' @param data Data set
...
#' AIC) and `model` (the best-fitting `ertte_model`, i.e. the one with
#' lowest AIC).
#'
...
ertte_select_distribution <- function(formula, data, candidates = c("exponential", "weibull", "lognormal", "loglogistic")) {
  ...
  fits <- lapply(candidates, function(dd) ertte_model(formula, data, dist = dd))
